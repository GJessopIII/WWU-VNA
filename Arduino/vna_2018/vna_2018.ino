...
void setup()
{
    ...
    delay(5);
    si5351.output_enable(SI5351_CLK1, SI5351_DRIVE_8MA); // Disable this clock LO_Q (not used).
    delay(5);
   ...
   /* si5351.set_ms_source(SI5351_CLK0, SI5351_PLLA);
    ...
    // Set Freq to F: "^FREQ,F$"
    dcp.addParser("FREQ", setFreq);
...
*/
void setFreq (char **values, int valueCount)
{
    unsigned long long freq;
    int i;
    if(valueCount != 2)
    {
        Serial.println("In setFreq, number of arguments is not correct.");
        return;  // Something is wrong if you end up here.
    }
    freq = atoi(values[1]);
    setOscillator(freq);
